...
from torch_geometric.transforms import RandomNodeSplit, Compose, LargestConnectedComponents, ToSparseTensor, NormalizeFeatures
...
def get_train_val_loaders(config, mode):
    ...
    if dataset == 'CIFAR-10':
        ...
        train_transform = Compose([LargestConnectedComponents(), SemiSplit(num_train_per_class=num_train_per_class, num_val_per_class = num_train_per_class, lcc = False)])
        valid_transform = train_transform
        train_data = Amazon(
            root=os.path.join(root_data, dataset), 
            name=dataset,
            pre_transform = train_transform
            )
        test_data = train_data
    elif dataset in ["CS", "Physics"]:
        train_transform = SemiSplit(num_train_per_class=num_train_per_class, num_val_per_class = num_train_per_class, lcc = False)
        valid_transform = train_transform
        train_data = Coauthor(
            root=os.path.join(root_data, dataset), 
            name=dataset, 
        ...
        train_transform = Compose([NormalizeFeatures(),SemiSplit(num_train_per_class=num_train_per_class, num_val_per_class = num_train_per_class, lcc = False)])
        valid_transform = train_transform
        train_data = Planetoid(
            root=os.path.join(root_data, dataset), 
            name=dataset,
            pre_transform = train_transform
            )   
    ...
    if search_space == 'NASBenchGraph':
        return train_data,test_data,test_data,train_transform, valid_transform
