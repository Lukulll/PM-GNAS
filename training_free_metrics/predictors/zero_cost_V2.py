...
from pruning_models.model_graph_jacob import *
from training_free_metrics.weight_initializers import init_net
...
class ZeroCostV2(Predictor):
    def __init__(self, config, batch_size=64, method_type='synflow'):
        ...
        num_classes_dict = { 'Computers': 10, 'Photo': 8, 'CS': 15, 'Physics': 5, 'Cora': 7, 'CiteSeer':  6, 'PubMed' : 3}
    ...
    def compute(self, **kwargs):
        ...
        elif self.config['search_space'] == 'NASBenchGraph':
            if self.config['tf'] == 'jacob_cov':
                network = get_jacob_graph_model_from_arch_str(kwargs['keep_mask'],link = self.config['link'], hp= self.config['hp'], in_dim = self.config['in_dim'], out_dim = self.config['out_dim'], dname = self.config['dataset'])
            else:
                network = get_graph_model_from_arch_str(kwargs['keep_mask'],link = self.config['link'], hp= self.config['hp'], in_dim = self.config['in_dim'], out_dim = self.config['out_dim'], dname = self.config['dataset'])
        elif self.config['search_space'] == 'NASBench301':
            pass
        else:
            raise ValueError(f'Not supporting this search space: {self.config["search_space"]}')
