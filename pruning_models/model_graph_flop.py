...
def get_graph_flop_model_from_arch_str(keep_mask,link, hp, in_dim, out_dim, dname):
    model = Net(
        link = link,
        hp = hp,
        in_dim = in_dim,
...
class Net(nn.Module):
    ...
    def forward(self, data):
        x = data.x
        adjs=  data.edge_index
        x = F.dropout(x, p=self.hp.dropout, training=self.training).float()
