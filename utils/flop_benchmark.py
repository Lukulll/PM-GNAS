...
from Graph_untils.split import SemiSplit

def get_dataset(dname):
    root_data = './benchmark_data/dataset'
    if dname[:3] == 'ogb':
        if dname == 'ogbn-arxiv':
            dataset = PygNodePropPredDataset(root='/content/drive/MyDrive/nas_graph/Data/data', name=dname)
        elif dname == 'ogbn-proteins':
            #dataset = PygNodePropPredDataset(root='/content/drive/MyDrive/NAS_An/NAS-Bench-Graph/nas_bench_graph/data', name=dname, transform = ToSparseTensor(attr = 'edge_attr', remove_edge_index = False))
            dataset = PygNodePropPredDataset(root='/content/drive/MyDrive/nas_graph/Data/data', name=dname, transform = ToSparseTensor(attr = 'edge_attr'))
        n_class = dataset.num_classes
        dataset = add_mask(dname, dataset)
    elif dname in ["CS", "Physics"]:
        dataset = Coauthor(root='/content/drive/MyDrive/nas_graph/Data/data', name=dname, pre_transform = SemiSplit(num_train_per_class=20, num_val_per_class = 30, lcc = False))
        n_class = dataset.num_classes
        dataset = dataset[0]
    elif dname in ["Photo", "Computers"]:
        train_transform = Compose([LargestConnectedComponents(), SemiSplit(num_train_per_class=4, num_val_per_class = 4, lcc = False)])
        valid_transform = train_transform
        train_data = Amazon(
            root=os.path.join(root_data, dname), 
            name=dname,
        ...
        dataset = Planetoid(root='/content/drive/MyDrive/nas_graph/Data/data', name=dname)
        n_class = dataset.num_classes
        dataset = dataset[0]
...
def get_model_infos(config,model, dataset):
    # model = copy.deepcopy( model )

    model = add_flops_counting_methods(model)
    # model = model.cuda()
    model.eval()
    train_loader, _, _, _, _ = get_train_val_loaders(config, mode='train')
    # cache_inputs = torch.zeros(*shape).cuda()
    # cache_inputs = torch.zeros(*shape)
    cache_inputs = train_loader[0]
    #print(cache_inputs)
...
def batch_counter_hook(module, inputs, output):
    ...
    inputs = inputs[0]
    batch_size = sum(inputs['train_mask'])
